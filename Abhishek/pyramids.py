import cv2
import numpy as np
# from Heart_beat import monitor
import time
...
def build_laplacian_pyramid(img, levels):
    ...
    for i in range(levels-1):
        ...
        # print(gaussian_pyramid[i+1])
        # print("=====================================")
        # print(upsampled)

        (height, width,depth) = upsampled.shape
        gaussian_pyramid[i] = cv2.resize(gaussian_pyramid[i], (height, width))
        (height1, width1, depth1) = gaussian_pyramid[i].shape
        # print(depth1)
        # print("=====================")


        # cv2.cvtColor(gaussian_pyramid[i],cv2.COLOR_BGR2RGB)
        # cv2.cvtColor(upsampled, cv2.COLOR_BGR2RGB)


        # diff = cv2.subtract(gaussian_pyramid[i],upsampled)
        diff=cv2.subtract(cv2.resize(gaussian_pyramid[i], (224, 224)).astype(np.float32),cv2.resize(upsampled, (224, 224)).astype(np.float32))
...
def build_video_pyramid(frames):
    lap_video = []
    # print(str(len(frames))+"   frame count")
...
def collapse_laplacian_video_pyramid(video, frame_ct):
    ...
    # height, width, layers = collapsed_video[3].shape
    # size = (width, height)
    # out = cv2.VideoWriter("data.avi", cv2.VideoWriter_fourcc(*'DIVX'), 30,size)
    # for i in range(len(collapsed_video)):
    #     # writing to a image array
    #     out.write(collapsed_video[i])
    # out.release()
